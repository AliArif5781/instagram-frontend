# Instagram Clone - Frontend

A modern Instagram clone frontend built with React, TypeScript, and Vite.

## 🚀 Features
- **Create Post**: Users can create an posts include photo and video and other users can also see his posts.
- **Create Story**: Users can create an story and follow users can see his story.
- **Send RealTime Messages**: Users can also send messages to his friend in real-time.
- **Searh Users**: Users can also search others users and see other users posts.
- **Follow & UnFollow**: User can also follow or unfollow other users and shown in followers and following.
- **Reels**: User can also see reels.
- **Feeds**: User can also see feeds.
- **SuggestedUsers**: User can also see suggested Users, like if user follow someone and the other user follow by someother user shown in suggested users section.
- **Authentication**: User login/signup with JWT tokens.
- **Media Management**: Secure image and video upload system powered by ImageKit CDN for optimized storage and delivery user can upload post 
- **Responsive Design**: Responsive on all devices.

## 🛠️ Tech Stack

- **Frontend Framework**: React 19 + TypeScript.
- **Backend Framework**: Express.
- **Database**: MongoDB.
- **Styling**: Tailwind CSS and sass.
- **State Management**: React Redux Toolkit.
- **Real-time**: Socket.io.

